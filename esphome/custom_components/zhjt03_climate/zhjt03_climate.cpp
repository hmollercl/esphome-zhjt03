...
void ZHJT03Climate::send_state_frame_() {
  uint8_t t = (uint8_t) roundf(this->target_temp_);
...
void ZHJT03Climate::transmit_frame_(uint16_t timer,
                                   ...
                                   uint16_t temp_mode,
                                   uint16_t footer) {
  auto *tx = remote_transmitter::global_remote_transmitter;
  if (tx == nullptr) return;

  tx->transmit([&](remote_transmitter::RemoteTransmitData *d) {
